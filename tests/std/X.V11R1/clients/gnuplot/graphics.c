/* $Header: graphics.c,v 1.1 87/09/11 08:19:57 toddb Exp $ */
/*
 *
...
double zero_center ();
    ...
    /* xtic, ytic = distances between tick marks in coordinates of
       (x, f(x)) as opposed to screen coordinates */
    ...
    /* This draws tick marks so that the Y ticks are at equal intervals,
       but the first X tick might be at a different distance from the
       beginning of the X axis than the other X ticks are from each other.
       I do not understand the reason for this, or whether it is a bug.
       --phr */

    least = (ymin < ymax) ? ymin : ymax;
    most = (ymin < ymax) ? ymax : ymin;

    /* Draw the tick marks on both axes; some more hair needs to
       be added here for the LOGSCALE case. */

    for (ticplace = least; ticplace <= most ; ticplace += ytic) { 
        char label[80];

	num2str (zero_center(ticplace, ytic), label);
	draw_tick_label (label, 'y', 0, map_y (ticplace));

	/* don't draw a tick mark at the beginning or end of the axis,
	   since that's where the other axis (or frame border) meets it.
	   We say "ticplace+ytic/100 > most" instead of "ticplace==most"
	   because of possible floating point roundoff error. */
	if (ticplace == least || ticplace + ytic/100. > most)
	    continue;
    ...
    for (ticplace = least; ticplace <= most ; ticplace += xtic) { 
	char label[80];

	/* Don't allow manually setting the X range to cause X axis
	   tick mark labels to fall outside the xmin:xmax interval.
	   Under autoscaling, the X scale would be chosen to make
	   sure that every tick mark (including the axis ends) got
	   labelled; but if the person has set the X range manually
	   and the program has chosen the tick mark spacing, we can't
	   guarantee that things will all fit nicely. */
	if (ticplace < xmin || ticplace - xtic/100. > xmax)
	  continue;

	num2str (zero_center(ticplace, xtic), label);
	draw_tick_label (label, 'x', map_x(ticplace), 0);

	/* Don't draw tick marks at ends of axis.  See above. */
	if (ticplace == least || ticplace + xtic/100. > most)
	    continue;
...
/* draw tick mark label STR on axis specified by AXIS (must be 'x' or 'y')
   at location X, Y.  If there is no driver routine for the current
   output device to label tickmarks, then don't do anything. */
static
draw_tick_label (str, axis, x, y)
     char *str;
     char axis;
     int x, y;
{
  struct termentry *t = &term_tbl[term];
  FUNC_PTR tick_label_func = (FUNC_PTR) term_tbl[term].tickmark_text;

  if (tick_label_func != NULL)
    (*tick_label_func) (str, axis, x, y);
}

/* Return NUM, unless NUM is very close to zero relative to TICSPACING
   in which case return 0.  This is to avoid labelling the origin
   "0.62346e-18" when the tick marks are 10.0 units apart. */
static double
zero_center (num, ticspacing)
     double num, ticspacing;
{
  double fabs ();
  if (fabs (num) < 1e-6 * fabs(ticspacing))
    return 0.0;
  return num;
}
