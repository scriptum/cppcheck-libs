/* $Header: command.c,v 1.1 87/09/11 08:19:47 toddb Exp $ */
/*
 *
...
static char user_helpfile[MAX_LINE_LEN];
...
static char testfile[MAX_LINE_LEN+1];
    ...
    else if (almost_equals(c_token, "h$elpfile")) {
        extern char *info_file_name;
	if (!isstring(++c_token))
	    int_error("expecting filename",c_token);
	quote_str (user_helpfile, c_token);
	info_file_name = user_helpfile;
	c_token++;
    }
	...
	if (END_OF_COMMAND) {
	  ...
	  int old_term = term;
	  /* make changing the terminal type cause the terminal
	     to be initialized, in case the init routine changes the
	     screen size.  That info should not really be encoded
	     in a static table and maybe something different shoulod
	     really be done.  --phr 13 May 1987 */
...
'helpfile', '[no]logscale', 'output', 'samples', 'terminal',\n\
'xrange', 'yrange', 'zero'",
		  c_token);
}
...
'dummy', 'function', 'helpfile', 'logscale', 'output', 'samples', 'terminal',\n\
...
eval_plots()
{
    ...
    /* make sure screen size is known before computing sample spacing */
    if (term_init == FALSE) {
      (*term_tbl[term].init)();
      term_init = TRUE;
    }
    ...
    if (mysamples <= 0)
        int_error ("Sampling rate must be > 0; is your screen set\
 up correctly?", NO_CARET);
...
do_help()
{
  int len = token[c_token].length;
  char *topic = (char *) alloca (len + 1);
  bcopy (&input_line[token[c_token].start_index], topic, len);
  topic[len] = '\0';
